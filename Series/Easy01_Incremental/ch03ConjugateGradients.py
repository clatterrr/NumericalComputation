    ...
    for j in range(sizeY):
        for i in range(sizeX):
            ...
            if j > 0:
                ...
                py = _aPlusX[i,j-1] * _precon[i,j-1]
                e = e - (px * px + tau * px * py)
...
def applyPreconditioner(a):
    ...
    for j in range(sizeY):
        for i in range(sizeX):
            ...
            if j > 0:
                t -= _aPlusX[i,j-1] * _precon[i,j-1] * dst[i,j-1]
    ...
    for j in range(sizeY):
        for i in range(sizeX):
            t = dst[i,j]
            if i < sizeX - 1:
                t -= _aPlusX[i,j] * _precon[i,j] * dst[i+1,j]
            if j < sizeY - 1:
                t -= _aPlusX[i,j] * _precon[i,j] * dst[i,j+1]
